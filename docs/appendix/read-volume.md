...
> **이 부록에서 하는 것**: 「테이블에 있는 행 나열」 도구를 1,000행짜리 표에 붙여 봅니다.
...
「테이블에 있는 행 나열」은 표를 통째로 읽어 옵니다. `지원기준` 표는 **다섯 행**이라 통째로 와도 아무 일이 없습니다.
...
1. **도구** 탭에서 **도구 추가**를 엽니다. 아래쪽 탭에서 **Excel Online (Business)** 의 **테이블에 있는 행 나열**을 고릅니다.

    ![도구 추가 — 같은 커넥터를 한 번 더](../../assets/appendix/read-volume-01.png)

    {: .note }
    **`지원기준`을 읽는 도구가 이미 있어도 새로 붙입니다.** 커넥터는 하나지만, 무엇을 읽을지 정해 둔 도구는 별개입니다.
...
2. **세부 정보**에서 **이름**을 아래로 바꿉니다.
    ...
    **완료 기준**: 도구 목록에 이름이 다른 도구가 둘이 된다.
    ...
    {: .important }
    커넥터가 붙여 주는 이름은 `테이블에 있는 행 나열 1` · `2` 라 사람도 구분하지 못합니다. MS 문서는 오케스트레이터가 **「이름과 설명」**으로 무엇을 부를지 판단한다고 적습니다 — 설명만 잘 써도 이름이 저 상태면 반쪽입니다.

3. **입력** 항목에서 `Table` 을 **`지원대상목록`** 으로 지정합니다. 나머지 칸은 `자기계발비_기준표.xlsx` 를 가리키게 사용자 지정 값으로 채웁니다.
...
7. **계약 테스트 창**에서 도구 상자를 눌러 **원시 출력**을 엽니다. `value` 배열의 길이를 셉니다.
    ...
    **완료 기준**: 배열이 `SD-0256` 에서 닫힌다. `SD-1000` 은 없습니다.
    ...
    {: .important }
**오류가 아닙니다.** `statusCode` 는 `200` 이고 상자에는 체크가 붙습니다. 성공적으로 끝난 호출인데 전부 오지 않았습니다.

    {: .note }
이 커넥터는 **기본적으로 최대 256행을 돌려줍니다.**[^learn-excel] 전부 가져오려면 페이지 매김을 켜야 하는데, 그 설정은 도구 화면에 없습니다 — 에이전트 흐름으로 내려가야 나옵니다.
...
**이 숫자가 질문 한 번에 오케스트레이터로 넘어간 양입니다.** 사용자는 「뭐가 있어?」 여섯 글자를 물었습니다. 지식 문서를 아무리 잘 정리해도, 도구가 이만큼을 실어 나르면 길이는 그쪽에서 정해집니다.
    ...
    {: .important }
색이 칠해진 것을 봅니다. `ItemInternalId` 의 `fd350d72-3a24-48f1-9eb9-d733ca02c3f4` 한 줄이 토큰 여러 개로 쪼개집니다. **쓰지도 않을 값이 자리를 차지합니다.**
...
9. 되돌립니다. 먼저 **지침에서 4번에 더한 줄을 지웁니다.**
    ...
    **완료 기준**: `지원대상 조회` 가 회색 **끄기** 상태가 된다.
    ...
    {: .note }
    지우지 않고 **끕니다.** 다시 해보고 싶을 때 켜면 됩니다.

    {: .warning }
    **반드시 되돌립니다.** 지침 줄과 도구를 남겨 두면 이후 테스트가 다른 조건에서 돌아갑니다.

---

## 더 해볼 것 — 에이전트 흐름에서 읽으면
...
같은 `지원대상목록` 을 흐름에서 읽어 보면 1,000행이 한 줄이 됩니다.
