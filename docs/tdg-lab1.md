...
<!-- 저작 메모 — 2026-09-14 초안. 촬영·실측 전이다.
     ...
     ★ 랩 간 의존
       ...
       - ⑥의 `지원단가 조회` 도구와 준비의 Excel 파일을 Lab 2 ③ 이후가 쓴다
...
- Excel 파일을 **본인 OneDrive for Business의 「문서」 폴더**에 업로드합니다.
...
26. 상단 **도구**에서 **도구 추가**를 열고, **커넥터** 탭에서 **Excel Online (Business)** 의 **테이블에 있는 행 나열**을 고른 뒤 **추가 및 구성**을 누릅니다.

27. **이름**과 **설명**을 아래로 바꿉니다.
...
28. **입력** 네 칸을 채웁니다.
    ...
    | File | 사용자 지정 값 | 폴더 아이콘으로 `자기계발비_기준표.xlsx` 선택 |
    | Table | 사용자 지정 값 | `지원기준` |

    `Table` 드롭다운이 비어 있으면 목록 아래 **새로 고침**을 누릅니다.

    {: .warning }
    **`Table`의 기본값은 「AI로 동적으로 채우기」입니다.** 그대로 두면 어느 표를 읽을지 오케스트레이터가 호출마다 정하고, `신청내역`을 읽고 지원단가를 답할 수 있습니다.

29. **사용할 자격 증명**이 **작성자가 제공한 자격 증명**인지 확인하고 **에이전트에 추가**를 누릅니다.

30. **새 테스트 세션**에서 물어봅니다.
