...
<!-- 저작 메모 — 2026-09-14 초안. 촬영·실측 전이다.
     ...
     ★ 제작자 결정(2026-09-14)
       ...
       - 2026-09-15: Forms 트리거는 NetworkIsolationAddressNotAllowed 오류가 계속 나서 부록 appendix/trigger.md 로 뺐다.
         대신 ⑥에서 같은 기록 도구를 대화로 부른다. 목록 기록이 ⑦·부록 sp-query-flow 의 전제라 트리거에 기대지 않게 했다
     ...
     ★ 랩 간 의존
       ...
       - 20번에서 만든 `경조신청_HGD` 에 ⑥ 31·33번이 기록하고, ⑦ 조회 도구와 부록 sp-query-flow 조회 흐름이 읽는다
     ...
     ⚠️ 실측할 것
       ...
       - CSV에서 목록 만들기의 스텝 수 · 열 형식 선택지 · 기본 「제목」 열 처리 · 내부 열 이름(부록 sp-query-flow 4번 필터 쿼리가 쓴다)
...
{: .time }
50분 타이머. 7단(에이전트 → 설정과 기준선 → SharePoint 지식 → 경조신청 목록 → 지침 → 기록 도구 → 조회 도구)으로 나눠 진행합니다.
...
30. 상단 탭 묶음(`+8` 등)에서 **개요**를 열고 **지침**의 **편집**을 누릅니다. 맨 아래에 아래 블록을 붙여넣습니다.
    ...
    붙여넣은 블록에서 `/경조 신청 기록` 글자를 지우고, 그 자리에 **`/`** 를 입력해 목록에서 `경조 신청 기록` 도구를 고릅니다. 도구 이름이 칩으로 바뀐 뒤 **저장**을 누릅니다.

    ![경조 신청 기록 칩이 들어간 지침](../assets/tdg1/30.png)
...
38. 상단 탭 묶음(`+8` 등)에서 **개요**를 열고 **지침**의 **편집**을 누릅니다. `[경조사 신청 처리]` 블록 맨 아래에 아래 한 줄을 더합니다.

    ```
    - 경조사 신청 내역을 묻는 요청은 /경조 신청 조회 도구로 조회한다.
    ```

    `/경조 신청 조회` 글자를 지우고 **`/`** 를 입력해 목록에서 `경조 신청 조회` 도구를 고른 뒤 **저장**을 누릅니다.

    ![경조 신청 조회 칩까지 들어간 지침](../assets/tdg1/38.png)

39. 테스트 창의 **새 테스트 세션**을 누르고 물어봅니다.

    ```
    경조사 신청 내역 보여줘
    ```

    ![경조 신청 조회가 완료로 기록된 활동과 답변](../assets/tdg1/39.png)
...
24번에서 붙인 지침에 30번의 `[경조사 신청 처리]` 블록과 38번의 조회 줄이 더해진 전문입니다.
...
`/경조 신청 기록`과 `/경조 신청 조회`는 화면에서 **칩**입니다. 통째로 붙여넣으면 글자가 되므로, 그 부분만 `/` 를 다시 입력해 목록에서 고릅니다.
