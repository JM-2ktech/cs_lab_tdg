...
<!-- 저작 메모 — 2026-09-14 초안. 촬영·실측 전이다.
     ...
     ★ 재료
       - 입문 Lab 5 ② 6~9 · ③ 12·13·15 · ④ 18~23 → Lab 1 ⑥ 28~32 · 이 랩 ③ 24~30
     ...
     ★ ⑥ 제작자 POC(2026-09-14) — 트리거만 추가하고 게시했더니 제대로 동작하지 않았다. 트리거 흐름이 응답 알림만 넘기기 때문이다.
       ...
       ⚠️ 3~6번 도구, 10~15번 흐름, 17번 이후는 미촬영.
...
- Lab 1 27번에서 OneDrive에 올린 `자기계발비_기준표.xlsx`를 그대로 씁니다.
...
1. [경조사 신청 양식 템플릿]({{ site.tdg.forms_template_url }})을 열고 오른쪽 위 **복제**를 누릅니다.
    ...
    ![복제 버튼이 있는 양식 템플릿](../assets/tdg2/1.png)
...
2. 복제한 양식의 제목을 아래로 바꾸고 문항 셋을 확인합니다. `HGD` 대신 본인 이니셜을 넣습니다.
    ...
    ![제목을 바꾼 복제 양식과 문항 셋](../assets/tdg2/2.png)
...
7. 개요의 **트리거**에서 **+ 트리거 추가**를 누르고, Microsoft Forms의 **새 응답이 제출되는 경우**를 고른 뒤 **다음**을 누릅니다.
    ...
    ![개요의 트리거 추가 버튼](../assets/tdg2/7.png)

    ![트리거 목록의 새 응답이 제출되는 경우](../assets/tdg2/7-2.png)

8. **로그인**의 Microsoft Copilot Studio와 Microsoft Forms에 초록 확인 표시가 있는지 봅니다. 표시가 없으면 **⋯**에서 연결을 새로 만든 뒤 **다음**을 누릅니다.

    ![로그인의 두 연결과 다음 버튼](../assets/tdg2/8.png)

9. **양식 ID**에서 2번에서 이름을 바꾼 양식을 고릅니다.

    ![양식 ID 목록에서 본인 양식 선택](../assets/tdg2/9.png)
    ...
    **`Use content from 본문` 줄은 지우지 않습니다.** 흐름이 넘긴 본문이 이 칩으로 에이전트에 전달됩니다.
    ...
    ![양식 ID와 추가 지침을 채운 트리거 추가 창](../assets/tdg2/9-2.png)
...
16. Copilot Studio로 돌아가 에이전트 우측 상단 **게시**를 누릅니다. **최신 버전 강제 적용**은 체크하지 않고 **게시**합니다.
    ...
    ![작성자 자격 증명 트리거 검토 항목이 뜬 게시 창](../assets/tdg2/16.png)
...
27. **도구 추가**에서 **Excel Online (Business)** 의 **테이블에 행 추가**를 고르고, Lab 1 30번과 같은 파일에 `Table`을 `신청내역`으로 지정합니다. 이름은 `신청내역 기록`으로 바꾸고 **에이전트에 추가**를 누릅니다.
